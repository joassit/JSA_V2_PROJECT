"""
Corre el candidate audit T1 (especializacion de Totales) contra el
historico REAL compartido (solo lectura) y persiste el resultado en
`candidate_audit_result` (schema propio `team_strength`).

Cero ingesta nueva -- solo lee `historical_game`/`historical_snapshot` via
data_sources/historical_readonly.py (rol `jsa_v2`, GRANT SELECT), y
calcula todo en memoria con analysis/totals_candidate_audit.py.
"""
...
import config
from analysis.totals_candidate_audit import evaluate_t1
...
def main() -> int:
    run_id = f"t1-totals-{datetime.now(timezone.utc).strftime('%Y%m%dT%H%M%S')}-{uuid.uuid4().hex[:8]}"
    ...
    print("Evaluando T1 (LOSO por temporada + bootstrap CI 500 resamples)...", file=sys.stderr)
    result = evaluate_t1(games, n_resamples=config.BOOTSTRAP_RESAMPLES)
    ...
    if result["meets_all_3_conditions"]:
        print("\nVEREDICTO: T1 cumple las 3 condiciones -- mejora real sobre el "
              "baseline de liga, no se adopta automaticamente, requiere "
              "revision explicita del usuario antes de cualquier siguiente paso.")
    else:
        print("\nVEREDICTO: T1 NO cumple las 3 condiciones -- no hay evidencia "
              "de que la proyeccion especifica del juego mejore sobre el "
              "baseline de promedio de liga para el mercado de Totales.")
    return 0
